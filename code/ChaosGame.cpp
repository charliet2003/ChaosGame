...
int main()
{
	...
	text.setString("Click on three points to draw the triangle and a fourth point as a starting point.");
	...
	while (window.isOpen())
	{
		...
		*/
		...
		if(points.size() > 0)
		{
			...
			vertex = vertices[rand() % vertices.size()];
			point = points[points.size() - 1];
			points.push_back(Vector2f((vertex.x + point.x) / 2, (vertex.y + point.y) / 2));
		}
